...
func main() {
	...
	specPath := os.Args[1]
	dbPath := filepath.Join("cmd", "fpf-rag", "fpf.db")
	if len(os.Args) >= 3 {
		dbPath = os.Args[2]
	}
	commitSHA := ""
	...
	chunks, err := internal.ChunkMarkdown(f)
	if err != nil {
	...
	if err := internal.BuildIndex(dbPath, filtered); err != nil {
		fmt.Fprintf(os.Stderr, "Error building index: %v\n", err)
		os.Exit(1)
	}

	if commitSHA != "" {
		if err := internal.SetMeta(dbPath, "fpf_commit", commitSHA); err != nil {
